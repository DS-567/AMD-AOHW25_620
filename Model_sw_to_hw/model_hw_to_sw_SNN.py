...
# (need to add an extra zero at front of threshold string as shared data size with bias in lif_neuron.vhd)
...
# write hidden layer biases to text file (hidden layer is called layer 1 on FPGA in case there are multiple hidden layers!)
...
for i in range(len(hidden_biases_list)):
    ...
    # join sign bit with the whole and fractional binary strings
    converted_str = sign_MSB + whole_str + fractional_str
...
# write output layer biases to text file (output layer is called layer 2 on FPGA as there is only one hidden layer!)
